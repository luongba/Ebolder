<template>
  <div class="container">
    ...
    <div class="flex items-center justify-center mt-4">
      <el-button @click="saveChangeTitle(detailTopic.id)">Change description</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    async deleteTopic(id) {
      try {
        let rs = await baseRequest.post(`/admin/delete-topic-grammar`, { id });
        if (rs.data.status == 200) {
          this.getAllTopic();
        }
      } catch (e) {
    ...
    async getAllQuestion() {
      try {
        let { data } = await baseRequest.get(`/admin/list-question-grammar`);
    ...
    async addTopic(id) {
      try {
        let { data } = await baseRequest.post(
          `/admin/add-question-to-topic-grammar`,
          {
    ...
    async removeTopic(id) {
      try {
        let { data } = await baseRequest.post(
          `/admin/remove-question-from-topic-grammar`,
          {
            idTopic: this.detailTopic.id,
            idQues: id,
          }
