            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                    ...
                    <div class="flex items-center">
                        <a
                            :href="`${ApiUrl}/admin/pronunciation-level-test/topic-detail/${item.id}`"
                            class="ml-2"
                        >
                            <el-button
                                size="small"
...
export default {
    ...
    methods: {
        ...
        async getAllTopic() {
            try {
                this.isLoading = true;
                let rs = await baseRequest.get(`/admin/list-topic-pronunciation?search=${this.textSearch}&page=${this.current}`);
                if (rs.data.status == 200) {
        ...
        async deleteTopic(id) {
            this.$confirm(
                ...
                .then(async () => {
                    try {
                        let rs = await baseRequest.post(
                            `/admin/delete-topic-pronunciation`,
                            { id }
                        );
                        if (rs.data.status == 200) {
                            this.getAllTopic();
