  ...
  methods: {
    ...
    async startExam() {
      if (this.currentClick !== null && !this.isExamDone) {
        window.location.href = `${$Api.baseUrl}/english-level-test/Reading?testId=${this.currentClick.reading_id}&v=${this.currentClick.vocabulary_id}&g=${this.currentClick.grammar_id}&l=${this.currentClick.listening_id}&s=${this.currentClick.speaking_id}&r=${this.currentClick.reading_id}&w=${this.currentClick.writing_id}&t=${this.currentClick.talking_id}&historyId=${this.history_id}&examId=${this.currentClick.id}&exam=true1`;
      }else {
        window.location.href = `${$Api.baseUrl}/exam/result/${this.history_id}&v=${this.currentClick.vocabulary_id}&g=${this.currentClick.grammar_id}&l=${this.currentClick.listening_id}&s=${this.currentClick.speaking_id}&w=${this.currentClick.writing_id}&t=${this.currentClick.talking_id}&r=${this.currentClick.reading_id}&historyId=${this.history_id}&exam=true1`;
      }
    },
  },
  created() {
